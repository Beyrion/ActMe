...
从 1.3.0 开始，循环预算提高到更适合报告、网页调研和多文件生成的规模，同时仍保留去重和上限保护。对于“模型空回复且没有任何输出文件”的 pass，Repository 会自动触发恢复提示，让 Agent 继续生成最终回复或继续完成用户要求的结果，而不是把空消息交给用户。
...
- Python 返回空回复且没有文件时，App 会要求 Agent 继续，不让用户手动补一句“继续”。
