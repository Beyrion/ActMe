...
2. 内置 Python：让 Agent 能做确定性计算、数据处理、Excel 读写、通用文件生成和脚本复用。
...
- 搜索多个网页，浏览权威来源，用 Python 清洗和汇总数据，生成 Excel 或 PDF。
...
- Python 沙箱允许导入标准库和已安装包，文件写入限制在工作区内，并限制进程、native code、包安装和系统 shell 等危险能力。
